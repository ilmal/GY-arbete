...
def grab_data_func(LOWER_DATE = "2022-05-11 00:00:00",   
  ...
  for index, row in df.iterrows(): # dropping all data that doesn't align with my set date above
    if lower_limit_day > dt.datetime.strptime(row["time"], '%Y-%m-%d %H:%M:%S'): # dropping all data before the lower limit
      df.drop(index, inplace=True)

    if upper_limit_day < dt.datetime.strptime(row["time"], '%Y-%m-%d %H:%M:%S'): #dropping all data after the upper limit
      df.drop(index, inplace=True)
